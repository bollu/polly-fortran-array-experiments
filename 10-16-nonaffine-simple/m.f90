...
SUBROUTINE f(arrin, arridx,  arrout, n)

  IMPLICIT NONE

! Subroutine arguments:
! --------------------
  REAL(KIND=wp), INTENT(IN)  :: arrin(:)
  REAL(KIND=wp), INTENT(IN)  :: arridx(:)
  REAL(KIND=wp), INTENT(OUT) :: arrout(:)
  INTEGER, INTENT(IN) :: n
...
!               (i.e. once the global fields and the block fields are on the GPU)
  !$acc data present(fld, fld_b, mind_ilon, mind_jlat)
  !$acc parallel
...
PROGRAM p
    USE m
    INTEGER :: i
    REAL(kind=wp) arrin(10), arridx(10), arrout(10)
    DO i = 1, 10
        arrin(i) = i
        arridx(i) = 2
        arrout(i) = -42
